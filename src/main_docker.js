$(function () {
    ...
    $('#createImage').on('click', function () {
        ...
        $('#ret').html('<p>发送中...</p>')
        var imagine_url = docker_url+"/mj/submit/imagine"
        var data = {
            "base64Array": [],
            "notifyHook":"",
            "prompt": prompt,
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data)
                if(data.code == 1){
                    resultId = data.result; //获取提交的任务id
                    Swal.fire({
                        title: '提示词已提交',
    ...
    //获取生成的图片:已经完成
    function getPic(){
        var get_url = docker_url+"/mj/task/list-by-condition";
        var data = {"ids": [resultId]};
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                //只显示第一张图片
                var picData = result[0]

                $('#list').html('')

                if(picData.status == "SUCCESS"){
                    h = '<p style="padding:20px 0px;font-size: 16px;">提示词：' +picData.properties.finalPrompt + '</p><br/>'
                    h += '<a target="_blank" href="' + picData.imageUrl + '"><img width="700px" src="' + picData.imageUrl + '"></a><br/>'
                    $('#list').html('')
                    $('#list').html(h)
                    ...
                    for(var i=1; i<=4; i++) {
                        ...
                        $a.one("click", function () {
                            //begin --- 生成放大图片
                            var upscale_url = docker_url+"/mj/submit/change";
                            var upscale_data = {
                                "action": "UPSCALE",
                                "index": $(this).attr("id"),
                                "notifyHook": "",
                            ...
                            var getUpscale_resultId = null;
                            console.log("发送数据的数据内容：")
                            console.log(JSON.stringify(upscale_data))
                            $.ajax({
                                url:upscale_url,
                                type:'post',
                                data: JSON.stringify(upscale_data),
                                contentType: "application/json;charset=utf-8",
                                dataType: "json",
                                success: function (res) {
                                    console.log("图片放大请求的结果")
                                    console.log(res)
                                    getUpscale_resultId = res.result;
                                }
                            });
                            //end --- 生成放大图片
                            ...
                            //获取放大图片的地址
                            setTimeout(function () {
                                var data_index = {"ids": [getUpscale_resultId]};
                                $.ajax({
                                        url: get_url,
                                        type: 'post',
                                        ...
                                        dataType: "json",
                                        success: function (result) {
                                            console.log("获取放大后的图片：")
                                            console.log(result)
                                            //只显示第一张图片
                                            var picData = result[0]
                                            if(picData.status == "SUCCESS"){
                                                //获取第一张图片
                                                window.open(picData.imageUrl, "_blank", "width=1000, height=800, top=100, left=100");
                                            }
                                        }
                                    }
                                );
                    ...
                    var res = '<p style="padding:20px 0px;font-size: 16px;">请耐心等待，图片生成中：' + picData.progress + '</p><br/>'
                    $('#list').html(res);
                }
            }
        })
